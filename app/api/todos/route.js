...
export async function POST(req) {
  try {
    ...
    return new Response(JSON.stringify({ message: "To-Do saved." }), { status: 200 });
  } catch (error) {
    console.error(error);
    return new Response(JSON.stringify({ error: "Failed to save To-Do." }), { status: 500 });
  }
}
